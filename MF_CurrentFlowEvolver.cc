...
#include <float.h>
#include <string>
//math.h added for R(U) and eta(TMR) ( because eta(TMR) uses R(U) to have sens )
#include <math.h>

#include "nb.h"
#include "director.h"
#include "timedriver.h"
#include "MF_CurrentFlowEvolver.h"
...
OXS_EXT_REGISTER(MF_CurrentFlowEvolver);
      ...
      rkstep_ptr(NULL)
{
    ...
    if(HasInitValue("signal_profile"))
    {
        ...
        V_profile_opts.push_back(Nb_TclCommandLineOption("total_time",1));
        V_profile_cmd.SetBaseCommand(InstanceName(),
                                     director->GetMifInterp(),
                                     GetStringInitValue("signal_profile"),
    ...
    else
    {
        ...
        for(OC_UINT4m i=0; i<vrange.size(); i++)
        {
            range_list.Split(vrange[i].c_str());
            if(range_list.Count() != 3)
            {
                char buf[1024];
                Oc_Snprintf(buf,sizeof(buf),
                            "signal_range[%u] value must be a 3 element list"
                            " (actual element count: %d)",
                            i,range_list.Count());
                throw Oxs_ExtError(this,buf);
            }
            OC_BOOL err=0;
            v0 = Nb_Atof(range_list[0],err);
            if(!err) v1 = Nb_Atof(range_list[1],err);
            if(!err) {
                char* cptr;
                long check = strtol(range_list[2],&cptr,10);
                step_count = static_cast<OC_UINT4m>(check);
                if(cptr == range_list[2] || *cptr!='\0' || check<0
                        || static_cast<long>(step_count) != check) err=1;
            }
            if(err) {
                char buf[1024];
                Oc_Snprintf(buf,sizeof(buf),
                            "signal_range[%u] value is not a 3 element list"
                            " of 2 reals and a non-negative integer: %s",
                            i,vrange[i].c_str());
                throw Oxs_ExtError(this,buf);
            }
            if(step_count==0 || Vapp.empty())
            {
                Vapp.push_back(v0);
            }
            else
            {
                OC_REAL8m last_v1 = Vapp.back();
                if(fabs(last_v1-v0)>fabs(v0)*OC_REAL8_EPSILON*2)
                {
                    Vapp.push_back(v0);
                }
            }
            for(OC_UINT4m j=1; j<step_count; j++)
            {
                ...
                Vapp.push_back(((1-t)*v0) + (t*v1));
            }
            if(step_count>0)
                Vapp.push_back(v1);
        }
        vector<OC_REAL8m>::iterator it = Vapp.begin();
        while(it!=Vapp.end())
        {
            *it *= vmult;
            ++it;
	...
	for(int i=0;i<interfaces.size();i+=2){
		inter_names.push_back(interfaces[i]);
		inter_parameters.push_back(interfaces[i+1]);
	}
	
	FindRequiredInitValue("Links",links_parameters_buffer);
	DeleteInitValue("Links");
	
	for(int i=0;i<links_parameters_buffer.size();i+=2){
		linksNames.push_back(links_parameters_buffer[i]);
		vector<string> params_key_value = split(links_parameters_buffer[i+1]);
	...
	for(int i=0;i<linksNames.size();++i){
		if(!linksParameters[i].count("interface1") || !linksParameters[i].count("interface2"))
			throw Oxs_Ext::Error("Parameters of Link must have defined parameters \"interface1\" and \"interface2\".");
	}
	
	vector< map<string,string> > bdryParam;
	
	for(int i=0;i<inter_names.size();++i)
	{
		vector<string> params_key_value = split(inter_parameters[i]);
		map<string,string> params_map;
		for(int j=0;j<params_key_value.size();j+=2)
			params_map[params_key_value[j]] = params_key_value[j+1];
		bdryParam.push_back(CheckBoundaryParameters(inter_names[i],params_map));
	}
	for(int i=0;i<inter_names.size();++i)
	{
		RegisterAtlas(bdryParam[i]);
	}
	for(int i=0;i<inter_names.size();++i)
	{
    ...
    if(method.compare("rk2")==0) {
        ...
        rkstep_ptr = &MF_CurrentFlowEvolver::TakeRungeKuttaStep4;
    } else if(method.compare("rkf54m")==0) {
        rkstep_ptr = &MF_CurrentFlowEvolver::TakeRungeKuttaFehlbergStep54M;
    } else if(method.compare("rkf54s")==0) {
        rkstep_ptr = &MF_CurrentFlowEvolver::TakeRungeKuttaFehlbergStep54S;
    } else if(method.compare("rkf54")==0) {
        rkstep_ptr = &MF_CurrentFlowEvolver::TakeRungeKuttaFehlbergStep54;
    } else {
        throw Oxs_Ext::Error(this,"Invalid initialization detected:"
    ...
    mxH_output.Setup(this,InstanceName(),"mxH","A/m",1,
                     &MF_CurrentFlowEvolver::UpdateDerivedOutputs);
    spin_torque_output.Setup(this,InstanceName(),"Spin torque","1/s",0,
                             &MF_CurrentFlowEvolver::UpdateSpinTorqueOutputs);
    field_like_torque_output.Setup(this,InstanceName(),"Field Like torque","1/s",0,
                                   &MF_CurrentFlowEvolver::UpdateSpinTorqueOutputs);
    conductance_output.Setup(this,InstanceName(),"channel conductance","1/Ohm",0,
                             &MF_CurrentFlowEvolver::UpdateDerivedOutputs);
    current_density_output.Setup(this,InstanceName(),"current density","A/m^2",0,
                                 &MF_CurrentFlowEvolver::UpdateDerivedOutputs);
...
MF_CurrentFlowEvolver::MagneticAtlas * MF_CurrentFlowEvolver::getMagneticAtlasByRegion(string region)
{
	for(int i=0;i<magneticAtlases.size();++i)
		if(magneticAtlases[i].region == region)
			return & magneticAtlases[i];
	throw Oxs_Ext::Error("Can't find MagneticAtlas with region: \"" + region +"\".");
}
...
void MF_CurrentFlowEvolver::FillLinkList
												...
												map<string,string> & params_map)
{
	...
	{
		throw invalid_argument(string(bad_conversion.what()) + "Can't parse some mif args to number.");
	}
	catch(out_of_range & cant_find_key)
	{
		throw out_of_range(string(cant_find_key.what()) + "You have to specify the key.");
	}
	//OPTIONAL PARAMETERS
	try{
    ...
    ThreeVector helper1, helper2;

    mesh->Center((links.begin())->index1,helper1);
    mesh->Center((links.begin())->index2,helper2);
    OC_REAL8m acoord,bcoord,ccoord;
    cellsurf=0;
    acoord = helper1.x-helper2.x;
    bcoord = helper1.y-helper2.y;
    ccoord = helper1.z-helper2.z;

    if(acoord != 0)
        cellsurf=mesh->EdgeLengthY()*mesh->EdgeLengthZ();
    if(bcoord != 0)
    {
        if(cellsurf == 0)
            cellsurf=mesh->EdgeLengthX()*mesh->EdgeLengthZ();
        else
    ...
    {
        if(cellsurf == 0)
            cellsurf=mesh->EdgeLengthX()*mesh->EdgeLengthY();
        else
            throw Oxs_Ext::Error("Badly defined surfaces for magnetoresistance calculations in MF_CurrentFlowEvolver.");
    }
...
{
    if(!has_V_profile) return 1.0;

    int index;
    if((index = V_profile_opts[0].position)>=0) { // stage
        V_profile_cmd.SetCommandArg(index,stage);
    }
    if((index = V_profile_opts[1].position)>=0) { // stage_time
        V_profile_cmd.SetCommandArg(index,stage_time);
    }
    if((index = V_profile_opts[2].position)>=0) { // total_time
        V_profile_cmd.SetCommandArg(index,total_time);
    }
    ...
    double result;
    V_profile_cmd.GetResultListItem(0,result);
    V_profile_cmd.RestoreInterpResult();

    return static_cast<OC_REAL8m>(result);
}

void MF_CurrentFlowEvolver::UpdateDerivedOutputs(const Oxs_SimState& state)
{   // This routine fills all the MF_CurrentFlowEvolver Oxs_ScalarOutput's to
    ...
    if(!state.GetDerivedData("Max dm/dt",max_dm_dt_output.cache.value) ||
            ...
            (mxH_output.GetCacheRequestCount()>0 && mxH_output.cache.state_id != state.Id())) {
...
//ComputeConductance(state);
		...
		for(int linkIndex=0;linkIndex<boundaryLinks.size();++linkIndex)
		{
			for(vector<Oxs_STT_NP_LinkParams>::const_iterator it=boundaryLinks[linkIndex].links.begin(); it!=boundaryLinks[linkIndex].links.end(); ++it)
			{
				...
				cur[it->index1].Set(0.0,0.0,it->conductance*Signal/(mesh->EdgeLengthX()*mesh->EdgeLengthY()));
				cur[it->index2].Set(0.0,0.0,it->conductance*Signal/(mesh->EdgeLengthX()*mesh->EdgeLengthY()));
			}
			if ( conductance == 0 )
			{
				throw Oxs_Ext::Error(this,
									 "MF_CurrentFlowEvolver::UpdateDerviedOutputs:"
...
void MF_CurrentFlowEvolver::UpdateSpinTorqueOutputs(const Oxs_SimState& state)
{
...
{
    if ( state.stage_number < Vapp.size()) //indexed from 0 to size-1
        return Vapp[state.stage_number];
    else
        return Vapp[Vapp.size()-1];
};

OC_BOOL MF_CurrentFlowEvolver::Init()
{
    // Setup outputs
...
void MF_CurrentFlowEvolver::Calculate_dm_dt(const Oxs_SimState& state_,
...
{   // Imports: state, mxH_, pE_pt_
    ...
    const Oxs_RectangularMesh* mesh
        = dynamic_cast<const Oxs_RectangularMesh*>(state_.mesh);
...
void MF_CurrentFlowEvolver::CheckCache(const Oxs_SimState& cstate)
{
    ...
    if(!cache_good) {
        throw Oxs_Ext::Error(this,
                             "MF_CurrentFlowEvolver::CheckCache: Invalid data cache.");
    }
}


void
MF_CurrentFlowEvolver::AdjustState
(OC_REAL8m hstep,
 OC_REAL8m mstep,
 ...
 OC_REAL8m& norm_error) const
{
    ...
    // to MF_CurrentFlowEvolver::Step() takes 2 half-steps, and it is the
    ...
    // in by the MF_CurrentFlowEvolver::NegotiateTimeStep() method.
}


void MF_CurrentFlowEvolver::UpdateTimeFields
...
void MF_CurrentFlowEvolver::NegotiateTimeStep
...
{   // This routine negotiates with driver over the proper step size.
    ...
    if(!cstate.GetDerivedData("Max dm/dt",max_dm_dt)) {
        throw Oxs_Ext::Error(this,
                             "MF_CurrentFlowEvolver::NegotiateTimeStep: max_dm_dt not cached.");
    }
    ...
    if(!cstate.GetDerivedData("Timestep lower bound",
                              timestep_lower_bound)) {
        throw Oxs_Ext::Error(this,
                             "MF_CurrentFlowEvolver::NegotiateTimeStep: "
...
OC_BOOL
MF_CurrentFlowEvolver::CheckError
...
void MF_CurrentFlowEvolver::TakeRungeKuttaStep2
(OC_REAL8m stepsize,
...
{   // This routine performs a second order Runge-Kutta step, with
    ...
    if(!endstate->AddDerivedData("Timestep lower bound",
        ...
        throw Oxs_Ext::Error(this,
                             "MF_CurrentFlowEvolver::TakeRungeKuttaStep2:"
...
void MF_CurrentFlowEvolver::TakeRungeKuttaStep4
(OC_REAL8m stepsize,
 Oxs_ConstKey<Oxs_SimState> current_state_key,
 const Oxs_MeshValue<ThreeVector>& current_dm_dt,
 Oxs_Key<Oxs_SimState>& next_state_key,
...
void MF_CurrentFlowEvolver::RungeKuttaFehlbergBase54
(RKF_SubType method,
 OC_REAL8m stepsize,
 Oxs_ConstKey<Oxs_SimState> current_state_key,
 const Oxs_MeshValue<ThreeVector>& current_dm_dt,
...
{   // Runge-Kutta-Fehlberg routine with combined 4th and 5th
    ...
    default:
        throw Oxs_Ext::Error(this,
                             "MF_CurrentFlowEvolver::RungeKuttaFehlbergBase54:"
...
void MF_CurrentFlowEvolver::TakeRungeKuttaFehlbergStep54
(OC_REAL8m stepsize,
 Oxs_ConstKey<Oxs_SimState> current_state_key,
 const Oxs_MeshValue<ThreeVector>& current_dm_dt,
 Oxs_Key<Oxs_SimState>& next_state_key,
...
void MF_CurrentFlowEvolver::TakeRungeKuttaFehlbergStep54M
(OC_REAL8m stepsize,
 Oxs_ConstKey<Oxs_SimState> current_state_key,
 const Oxs_MeshValue<ThreeVector>& current_dm_dt,
 Oxs_Key<Oxs_SimState>& next_state_key,
...
void MF_CurrentFlowEvolver::TakeRungeKuttaFehlbergStep54S
(OC_REAL8m stepsize,
 Oxs_ConstKey<Oxs_SimState> current_state_key,
 const Oxs_MeshValue<ThreeVector>& current_dm_dt,
 Oxs_Key<Oxs_SimState>& next_state_key,
...
OC_REAL8m MF_CurrentFlowEvolver::MaxDiff
...
void MF_CurrentFlowEvolver::AdjustStepHeadroom(OC_INT4m step_reject)
                            ...
                            Oxs_Key<Oxs_SimState>& next_state_key)
{
    ...
    if(next_timestep<=0.0 ||
        ...
        if(cstate.stage_number==0
            ...
            throw Oxs_Ext::Error(this,
                                 "MF_CurrentFlowEvolver::Step; Programming error:"
    ...
    if(!cstate.GetDerivedData("dE/dt",current_dE_dt)) {
        throw Oxs_Ext::Error(this,"MF_CurrentFlowEvolver::Step:"
