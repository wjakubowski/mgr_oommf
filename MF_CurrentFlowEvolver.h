...
/////////////// eta(u)
    ...
    //OC_REAL8m A; // surface of interface A=linksss[0].links.size()*cellsurf; line 1200
    int fit_type;
...
class Boundary {
	...
	Boundary(string boundaryName, Oxs_ScalarField * bdry, OC_REAL8m bdry_value, String bdry_side, MagneticAtlas * magAtl)
	{
		...
		this -> ptrBdry = bdry;
		this -> bdry_value = bdry_value;
		this -> bdry_side = bdry_side;
		this -> ptrMagAtl = magAtl;
	}
	Oxs_ScalarField * getBdry()
	{
		return ptrBdry;
	}
	static bool sorter(const Boundary & left, const Boundary & right)
	{
		if(left.bdry_value > right.bdry_value)
			return true;
		else if(left.bdry_value < right.bdry_value)
			return false;
		else
			if(left.bdry_side == "-" && right.bdry_side == "+")
				return true;
			else
				return false;
	}
...
class LinksBetweenTwoInterfaces {
	public:
		...
		OC_REAL8m R_p;
		OC_REAL8m fit_param1;// = R_AP
		OC_REAL8m fit_param2;// = R_ap_param2
		OC_REAL8m fit_param3;// = R_ap_param3
	...
	MagneticAtlas * getMagneticAtlasByRegion(string region);
	Boundary * getBoundaryByName(string boundaryName);
	
	void RegisterBoundary(string & boundaryName,map<string,string> & boundryParameters);

	void LinkTwoSurfaces(const Oxs_RectangularMesh* mesh,
								map<string,string> & params_map);
									
	
	//###################################################### 
	
    void FillLinkList(const Oxs_RectangularMesh* mesh);

    mutable vector<OC_REAL8m> Vapp;
    OC_REAL8m GetSignal(const Oxs_SimState& state);
    OC_REAL8m Signal;
